...
module.exports = function(where, table, values){
  var buildConditions = function(where, condition, column, joiner, parent = ''){
    const conditionData = [];
    joiner = joiner || ' and ';
    if (column) {
        const columnValue = `"${parent.slice(0,-1) !== column ? parent : ''}${column}"`.split('.').join('"."');
        column = `JSON_UNQUOTE(JSON_EXTRACT(attributes,'$.${columnValue}'))`;
    ...
    for (var key in where) {
      ...
      if (typeof where[key] == 'object' && !(where[key] instanceof Date) && !Buffer.isBuffer(where[key]) && where[key] !== null) {
        ...
        // Key is column
          ...
          (result = buildConditions(where[key], condition, key, undefined, `${parent}${key}.`)) && conditions.push(result);
      ...
      // Key is a column
          ...
          const keyColumnValue = `"${parent}${key}"`.split('.').join('"."');
          conditionData.push(keyColumnValue);
          conditions.push(
              helpers.get(condition).fn(
                  // utils.quoteObject(key, table)
                  `JSON_UNQUOTE(JSON_EXTRACT(attributes,'$.${keyColumnValue}'))`
                  , where[key] === null ? null : utils.parameterize(where[key], values)
                  , values
                  , table
                  , where[key]
    ...
    if (conditions.length >= 1 && values.length) {
      const conditionValues = values.slice(conditions.length * -1);
      for (let i=0; i<conditions.length; i++) {
        if (conditions[i].substr(-3) === "= ?") {
          conditions[i] = `(${conditions[i]} or JSON_SEARCH(attributes, 'one', '${conditionValues[i]}', NULL, '$.${conditionData[i]}') is not null)`;
        }
      }
    }

...
module.exports = function(where, table, values){
  var buildConditions = function(where, condition, column, joiner, parent = ''){
    joiner = joiner || ' and ';
    if (column) column = `JSON_UNQUOTE(JSON_EXTRACT(attributes,'$.${parent.slice(0,-1) !== column ? parent : ''}${column}'))`; //utils.quoteObject(column, table);
    ...
    for (var key in where) {
      ...
      // Key is a column
        ...
        conditions.push(
          ...
          `JSON_UNQUOTE(JSON_EXTRACT(attributes,'$.${parent}${key}'))`
          , where[key] === null ? null : utils.parameterize(where[key], values)
          , values
          , table
          , where[key]
